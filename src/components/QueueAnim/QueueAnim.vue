<script>
import TweenOne, { ticker } from "../TweenOne";
import {
  toArrayChildren,
...
export default {
  props: {
    ...
    animatingClassName: {
      type: Array,
      default: () => ["queue-anim-entering", "queue-anim-leaving"]
    },
    onEnd: {
      type: Function,
      default: noop
  ...
  methods: {
    getChildrenToRender(child) {
      ...
      let animation;
      const isFunc = typeof child.tag === "function";
      const forcedJudg = isFunc ? {} : null;
      if (isFunc) {
        Object.keys(child.tag).forEach(name => {
          if (typeDefault.indexOf(name) === -1) {
            forcedJudg[name] = child.tag[name];
          }
        });
      }
      // 处理出场
      if (i >= 0) {
        ...
        const order =
          (leaveReverse ? this.keysToLeave.length - i - 1 : i) -
        ...
        if (this.tweenToEnter[key] && !forcedReplay) {
          // 如果是已进入的，将直接返回标签。。
          return this.createElement(TweenOne, {props:{
            key,
            component: child.tag,
            forcedJudg,
            componentProps: child.children
          }});
        }
      }
      const paused =
        this.keysToEnterPaused[key] && !this.keysToLeave.indexOf(key) >= 0;
      animation = paused ? null : animation;
      const tag = this.createElement(TweenOne, {
        props: {
          key,
          component: child.tag,
          forcedJudg,
    ...
    getTweenData(key, i, type) {
      ...
      startAnim =
        type === "enter" && (props.forcedReplay || !this.unwantedStart[key])
          ? startAnim
          : null;
      ...
      if (Array.isArray(ease)) {
        ...
        ease = TweenOne.easing.path(
          `M0,100C${ease[0]},${100 - ease[1]},${ease[2]},${100 -
            ease[3]},100,0`,
    ...
    getTweenEnterOrLeaveData(key, i, delay, type) {
      ...
      const onStart = (type === "enter"
        ? this.enterBegin
        : this.leaveBegin
      ).bind(this, key);
      const onComplete = (type === "enter"
        ? this.enterComplete
        : this.leaveComplete
      ).bind(this, key);
      ...
      return [animateData.startAnimate, animateData.animation].filter(
        item => item
      );
    },
    ...
    getAnimData(props, key, i, enterOrLeave, startOrEnd) {
      ...
      return props.animConfig
        ...
        : this.getTweenType(
            transformArguments(props.type, key, i)[enterOrLeave],
            startOrEnd
          );
    },
    ...
    performEnterBegin(key) {
      ...
      // 触发render
      this.$set(this.childrenShow, key, true);
    },
    ...
    enterComplete(key, e) {
      ...
      elem.className = elem.className
        .replace(this.$props.animatingClassName[0], "")
        .trim();
    ...
    leaveComplete(key, e) {
      ...
      if (!needLeave) {
        const currentChildren = this.$slots.default;
        this.children = currentChildren;
        this.childrenShow = childrenShow;
      }
      const elem = e.target;
      elem.className = elem.className
        .replace(this.$props.animatingClassName[1], "")
        .trim();
    ...
    componentWillReceiveProps(nextProps) {
      ...
      if (this.children.length) {
         ...
         */
        const leaveChild = this.children.filter(
      ...
      this.keysToEnterPaused = {};
      const emptyBool =
        !nextChildren.length && !currentChildren.length && this.children.length;
      /**
